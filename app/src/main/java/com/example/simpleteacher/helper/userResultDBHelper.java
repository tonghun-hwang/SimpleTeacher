...
public class userResultDBHelper extends SQLiteOpenHelper {
    public static final String TABLE_BASE = "table_base";
    public static final String TABLE_SESSION_LAST_BASE = "table_last_base";
    public static final String RESULT_DIAGNOSTIC = "result_diagnostic";
    public static final String RESULT_DIAG_DETAIL = "result_diag_detail";
    public static final String RESULT_DIAG_MAIN = "result_diag_main";
    ...
    public userResultDBHelper(Context context, String dbName, int version) {
    ...
    public void onCreate(SQLiteDatabase db) {
        createSimpleWordListTable(db, TABLE_BASE);
        createSimpleWordListTable(db, TABLE_SESSION_LAST_BASE);
        createWordListTable(db, RESULT_DIAGNOSTIC);
        createCategoryResultTable(db, RESULT_DIAG_DETAIL);
        createCategoryResultTable(db, RESULT_DIAG_MAIN);
    }

    @Override
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        db.execSQL("DROP TABLE IF EXISTS " + RESULT_DIAGNOSTIC);
        db.execSQL("DROP TABLE IF EXISTS " + TABLE_BASE);
        db.execSQL("DROP TABLE IF EXISTS " + TABLE_SESSION_LAST_BASE);
        onCreate(db);
    }
    ...
    public void createMainCategoryResultTable(SQLiteDatabase db, String tableName) {
        db.execSQL("DROP TABLE IF EXISTS " + tableName);
        db.execSQL("CREATE TABLE IF NOT EXISTS " + tableName +
                " (ID INTEGER PRIMARY KEY AUTOINCREMENT, " +
                "SESSION INTEGER," +
    ...
    public boolean replaceData(SQLiteDatabase db, String[] data, int id) {
        ...
        if (data.length > 0) {
            ...
            res = db.replace(RESULT_DIAGNOSTIC,
                    null, contentValues);
        }
        if (res == -1) {
            return false;
    ...
    public void createSimpleWordListTable(SQLiteDatabase db, String tableName) {
        db.execSQL("DROP TABLE IF EXISTS " + tableName);
        db.execSQL("CREATE TABLE IF NOT EXISTS " + tableName +
                " (ID INTEGER PRIMARY KEY AUTOINCREMENT, " +
                "WORD TEXT," +
    ...
    public boolean replaceSimpleData(SQLiteDatabase db, String tableName, String word, double difficult, int[] integers) {
        Log.d(TAG, "replaceData (mode): ");
        ...
        if (word != null) {
            ContentValues contentValues = new ContentValues();
            contentValues.put(ID, integers[0]);
            contentValues.put(WORD, word);
            contentValues.put(CATEGORY, integers[1]);
            contentValues.put(DIFFICULT, difficult);
            contentValues.put(DATE, update);
            res = db.replace(tableName,
                    null, contentValues);
        }
